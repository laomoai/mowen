...
Skill：\`/agent/mowen/SKILL.md\``,
    ...
    components: {
      ...
      schemas: {
        ...
        FieldMapping: {
          ...
          additionalProperties: {
            ...
            properties: {
              ...
              field_type: { type: 'string', enum: ['text', 'longtext', 'number', 'currency', 'percent', 'email', 'url', 'date', 'datetime', 'checkbox', 'select', 'image', 'link', 'totp', 'password', 'running_balance'] },
            },
          },
        ...
        RunningBalanceConfig: {
          type: 'object',
          description: '累计余额的固定配置。余额 = 期初余额 + 累计收入 - 累计支出。',
          required: ['version', 'kind', 'opening_balance', 'income_field', 'expense_field', 'order_field', 'tie_breaker', 'null_as_zero', 'precision'],
          properties: {
            version: { type: 'integer', enum: [1] },
            kind: { type: 'string', enum: ['running_balance'] },
            opening_balance: { type: 'string', pattern: '^-?\\d+(\\.\\d{1,2})?$', example: '10000.00' },
            income_field: { type: 'string', nullable: true, description: '数字或货币字段；与支出字段至少填一个。' },
            expense_field: { type: 'string', nullable: true, description: '数字或货币字段；与收入字段至少填一个。' },
            order_field: { type: 'string', description: 'ID（id）、日期或日期时间字段。' },
            order_direction: { type: 'string', enum: ['asc', 'desc'], default: 'asc', description: '累计计算方向；旧配置默认 asc。' },
            tie_breaker: { type: 'string', enum: ['id'] },
            null_as_zero: { type: 'boolean', enum: [true] },
            precision: { type: 'integer', enum: [2] },
          },
        },
        SpaceSummary: {
          type: 'object',
    ...
    paths: {
      ...
      '/api/tables/{tableName}/fields': {
                      ...
                      data: {
                        ...
                        items: {
                          ...
                          properties: {
                            ...
                            formula_config: { allOf: [{ $ref: '#/components/schemas/RunningBalanceConfig' }], nullable: true },
                            virtual: { type: 'boolean', description: '计算字段为 true' },
                            read_only: { type: 'boolean', description: '客户端不得写入该字段' },
        ...
        post: {
          summary: 'Add field',
          description: '添加普通字段时执行 ALTER TABLE；添加 running_balance 时只创建只读虚拟字段元数据。每张表最多一个累计余额。',
          parameters: [{ name: 'tableName', in: 'path', required: true, schema: { type: 'string' } }],
          requestBody: {
            required: true,
            content: {
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    formula_config: { $ref: '#/components/schemas/RunningBalanceConfig' },
                  },
                },
              },
            },
      ...
      '/api/tables/{tableName}/fields/{colName}': {
        patch: {
          ...
          responses: {
            '200': { description: 'Updated successfully' },
            '400': { description: 'Invalid formula configuration or unsupported type conversion' },
            '409': { description: 'A dependent source field cannot be changed to an incompatible type' },
          },
        },
        delete: {
      ...
      '/api/notes': {
        ...
        post: {
          summary: 'Create note',
          description: 'Creates a new note. Content size is limited to 1MB. For scope=groups API keys, root notes must provide folder_id for an authorized workspace folder.',
          requestBody: {
            required: true,
            content: {
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    folder_id: { type: 'string', nullable: true, description: 'Optional workspace folder node id for a root note in the sidebar. Ignored when parent_id is set. Required for scope=groups API keys and must reference an authorized folder.' },
                  },
                },
              },
            },
          ...
          responses: {
            ...
            '400': { description: 'Invalid parent_id or missing/invalid folder_id for scoped root note creation' },
            '403': { description: 'Write forbidden or target folder is outside the API key scope' },
